...
void answerQuestion(stQuestion& question, short questionNumber, enLevel level, enOpType opType, short numberOfQuestions)
{
...
void showQuestionResult(int rightAnswer, int yourAnswer)
{
	bool isAnswerCorrect = isYourAnswerCorrect(rightAnswer, yourAnswer);
...
void answerQuestions(stQuizzInfo& quizInfo)
{
	cout << endl;
	for (short i = 1; i <= quizInfo.NumberOfQuestions; i++)
	{
		answerQuestion(quizInfo.Questions[i - 1], i, quizInfo.Level, quizInfo.OpType, quizInfo.NumberOfQuestions);

		showQuestionResult(quizInfo.Questions[i - 1].Result, quizInfo.Questions[i - 1].PlayerAnswer);
